/*
 * jw3b.dev v2 — KTHULHU's steppable audit pipeline (FR-006)  ·  creative-technologist
 * NON-3D per the approved design lock (anti-spectacle / flat cyan engineered panels).
 *
 * ✎ MOVED 2026-08-22 out of the Overmind flagship and onto the KTHULHU card, where it was always
 * true. It rendered for a day under the heading "Overmind" — a different system that shares the
 * word (see kthulhuPipeline.js for all three). Nothing here changed but its address and its name:
 * the pipeline was correct, the card it sat on was not.
 *
 * Renders KTHULHU's real four-phase pipeline across its TWO LANES — Cloudflare orchestration and
 * box execution — with its two real gates.
 *
...
export default function KthulhuPipeline() {
  const [step, setStep] = useState(0)
  const complete = isComplete(step)
  const detail = stepDetail(step)

  return (
    <section aria-labelledby="kthulhu-pipeline-title" className="mt-4 rounded-md border border-hairline bg-void/40 p-4">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          {/* No longer captioned "Overmind". This is KTHULHU's own pipeline, on KTHULHU's own card,
              named for what it is. The engine that shares the word lives in the Overmind flagship. */}
          <p className="font-mono text-[11px] uppercase tracking-label text-cyan">Inside the audit — KTHULHU&rsquo;s two lanes</p>
          <h3 id="kthulhu-pipeline-title" className="mt-1 font-display text-lg font-semibold text-content-primary">How an audit actually runs</h3>
        </div>
