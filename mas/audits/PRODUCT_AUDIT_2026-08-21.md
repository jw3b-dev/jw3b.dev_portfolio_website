...
| 34 | **The Bot Fight Mode toggle is dashboard-only, and the right switch is narrower than assumed.** `enable_js` is a distinct field from `fight_mode`, so **JavaScript Detections can be turned off while Bot Fight Mode stays on** — earlier notes assumed the two were inseparable. That single toggle removes the `/cdn-cgi/challenge-platform/…/jsd/…` script and the `cf_clearance` cookie behind finding 32, with no loss of bot protection. Ask: Security → Bots → **JavaScript Detections off**. | P2 | **OPEN — owner-gated (dashboard)** |
| ~~33b~~ | ~~superseded~~
