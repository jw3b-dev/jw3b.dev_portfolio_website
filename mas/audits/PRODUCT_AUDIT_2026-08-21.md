...
| 23 | 4 CSP console errors on **every page** (Cloudflare zone RUM injection vs our CSP) | P2 | 3 | W5+owner  **✎ RE-VERIFIED 2026-08-22 (post-fix): PARTIALLY closed, and I called it closed too early.** The Google tag gateway IS off — `window.dataLayer` absent, `_gcl_ls` gone, `npm run e2e:zone` 3/3 on the apex. That was the compliance half and it is genuinely fixed. But **Cloudflare Web Analytics is a SEPARATE zone setting and is still injecting** `static.cloudflareinsights.com/beacon.min.js`, which our CSP still blocks — so the apex still logs CSP console errors. Harmless (it writes nothing; the zone-posture guard passes for that reason) but FR-067's "zero console errors" is not met on the domain visitors use. **Remaining ask: Analytics → Web Analytics → off.** Note the guard could not catch this: it tests cookies, storage and tag execution, not console errors — and the CI console budget runs against workers.dev, which has no zone layer (finding 31). |
...
| 25 | ~~P1~~ → **P3** | **RE-DIAGNOSED 2026-08-22 — over-stated, the fourth time I have escalated something to owner-gated that was not blocking.** Verified on-chain, not inferred: the vault at `0x4f72…C240` holds **0.00012 ETH and is drainable**, and every selector the client flow needs is present in the deployed bytecode (`deposit()` `d0e30db0`, `withdraw()` `3ccfd60b`, `balances(address)` `27e235e3`). `contracts.js:52` already said so — *"compatible because the v2 attack calls deposit()/withdraw() (identical selectors) and the Worker verifies by raw eth_getBalance"*. **The CTF is attemptable today.** What is genuinely owner-dependent is much narrower: the bait is small, so the FIRST solver empties it and it needs re-arming. That case is already handled honestly — `ctfFlow` has a `vault-empty` phase and the UI says *"The vault has no bounty to drain right now"* rather than failing silently. **Ask reduces to: re-fund the vault after a capture.** A v2 redeploy is a nice-to-have, not a blocker. |
...
| 28 | P2 | **CLOSED — degrades honestly; owner-gated to go live** | Reclassified. XMTP is no longer absent: `@xmtp/browser-sdk` is a dependency, `useXMTP` + `xmtpFlow.js` are implemented, and `/messages` gates on `isEnabled('xmtp') && isEthAddress(XMTP_RECIPIENT)`. The flag is unset in `.env.production`, so the surface degrades rather than selling what it lacks. **Ask: a provisioned XMTP recipient address.**  **✎ RE-DIAGNOSED 2026-08-22: the ask was necessary but NOT sufficient.** The recipient address would not have produced a working channel — `connect-src` carried **no XMTP host**, so the SDK's own endpoints (`api.production.xmtp.network`, `api.dev.xmtp.network`, read out of the installed package rather than guessed) were CSP-blocked. Provisioning the address would have switched on a surface that failed on its first call: the same shape as finding 21, an owner ask that could not have worked when acted on. Hosts added and pinned by a test against the served header. **The flag is now the only thing between here and a working channel.** |
...
| 33 | ~~**No credential on this machine can change the jw3b.dev zone.**~~ **✎ WRONG — corrected 2026-08-23.** One can: a `cfat_` token sitting as free text (not a `KEY=` line) in the **v1 repo's** `.env`. It reads every zone setting and writes them — proven by PATCHing `challenge_ttl` back at its own value, 200. Every search had keyed on the name `CLOUDFLARE_API_TOKEN`; the same trap caught two sweeps in one session. **Grep the token shape, never the variable name.** A second claim was also wrong: that a token can never be widened to another account. False for user tokens — `cfut_` belongs to the user and its zone resources may span any account that user reaches. Both were stated with more confidence than the evidence carried. **The real limit is the plan, not any credential:** `jw3b.dev` is Free, so `PUT /zones/{id}/bot_management` returns 10400 while the same token writes other zone settings fine. See `docs/OPS.md`. | — | **CLOSED — the premise was false** |
| 34 | ~~**The Bot Fight Mode toggle is dashboard-only, and `enable_js` is separable.**~~ **✎ WRONG, corrected the same hour — both halves.** The API write works (not a plan limit), and JS Detections is **not** separable: `PUT bot_management {"fight_mode":true,"enable_js":false}` is refused with *"cannot enable Fight_Mode while EnableJS is disabled"*. That reason sat in the response's `messages[]` while only `errors[].code` was logged — the cause of three wrong conclusions in one session. The original assumption, that JS Detections cannot be turned off under Bot Fight Mode, was correct before it was "corrected". **The choice is binary: keep Bot Fight Mode and the disclosed `cf_clearance` cookie, or turn Bot Fight Mode off and lose bot protection entirely.** Owner's call; nothing changed. | P2 | **OPEN — owner decision, not a permission** |
| ~~33b~~ | ~~superseded~~
