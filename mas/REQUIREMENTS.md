...
| **FR-066 ★** | …**operate** every surface that claims to be operable, or read copy that stops claiming it. A framed third-party product is not an on-site capability. | `e2e/first-visit.spec.js` (thesis surfaces asserted to CHANGE when driven) · `KthulhuCorpus.test.jsx` · **KTHULHU CLOSED 2026-08-22** — its retrieval layer runs on-site against the same corpus, no product API involved. The "owner-gated on API access" note this row carried was a misdiagnosis of mine: the endpoints were deployed and uncalled. **Kointel still OPEN — re-diagnose before escalating to the owner.** | 04 | MUST |
