...
| **FR-066 ★** | …**operate** every surface that claims to be operable, or read copy that stops claiming it. A framed third-party product is not an on-site capability. | `e2e/first-visit.spec.js` (thesis surfaces asserted to CHANGE when driven) · `KthulhuCorpus.test.jsx` · `KointelGate.test.jsx` · **MET 2026-08-22 for all four flagships.** KTHULHU runs its retrieval layer on-site; Kointel runs its compliance rule on-site. Neither needed the product API this row was gated on for a month — that note was a misdiagnosis, and the second one was found only because the first was corrected. | 04 | MUST |
