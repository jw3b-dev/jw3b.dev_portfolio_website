...
- ~~The hero screen is the one surface P5-02 cannot see.~~ **✎ DONE 2026-08-22.** `POST /funnel` accepts a client-reported event and `funnelBeacon.js` sends one when the visitor EDITS the sample — not on mount, because arriving at a page that renders a pre-filled result is not "running a screen", and counting it would inflate the exact number this exists to make honest. ADR-P5-01 intact: two enum values, no identifier, deduped **in memory** per page load rather than in storage, fire-and-forget, and origin-gated so it cannot repeat the CORS console-error mistake the liveness badge made. The boundary test for the new route found a real defect: `normalizeEvent(null)` threw, in a function contracted to return null for bad input.
