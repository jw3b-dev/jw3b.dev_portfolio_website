...
- ~~The register has no history.~~ **✎ DONE 2026-08-22.** `src/data/claims-lock.json` + `npm run claims-lock`, blocking in CI: any change to a cleared claim's value or evidence pointer must update the lock in the SAME commit, which forces the before and after into the diff. It flags the interesting case explicitly — a value that moved while its evidence pointer did not. Not derived from git on purpose: `actions/checkout` fetches depth 1, so a history check would silently no-op in CI, which is worse than none. It cannot judge whether a figure is TRUE — nothing mechanical can — only make a change impossible to make quietly, which is what the register was missing. Red-witnessed against the real case (`4c6f758` changed a cleared value, correctly and invisibly).
