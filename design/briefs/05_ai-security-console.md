...
- ~~A shareable run.~~ **✎ DONE 2026-08-22 — and built differently from this line.** It proposed "source hash + findings, no source stored", which produces a link whose recipient can read claimed findings and verify none of them, on the surface whose whole argument is that results should be reproduced rather than believed. The link carries the SOURCE in the URL **fragment** instead: a fragment is never transmitted to any server, so nothing is stored, no id exists, no retention question arises — and the recipient's browser re-derives the findings itself. Refuses above 8,000 characters with a stated reason rather than truncating (a shortened contract screens differently from the sender's).
