...
- ~~A defined stacking order at 390px.~~ **✎ DONE 2026-08-22.** The z tokens existed and two fixed components bypassed them with raw `z-40`/`z-50` that happened to match — and the consent banner **tied** with the nav at 40 with no defined winner. Now `nav < overlay < toast`, banner at `toast` because a blocking compliance notice must be able to cover the nav. A test fails on any raw `z-<number>`, since a raw value renders identically today and silently forks the token system. Red-witnessed.
